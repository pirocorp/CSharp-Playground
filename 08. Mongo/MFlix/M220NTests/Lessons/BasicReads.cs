...
namespace M220NLessons
{
    ...
    public class BasicReads
    {
        ...
        public async Task GetOneMovieAsync()
        {
             ...
             * name as the first parameter and the value as the second:
             */
            ...
            var betterFilter = Builders<Movie>.Filter
                .Eq(m => m.Title, "The Princess Bride");

            /*
             * Now we call Find, passing in the filter, and casting the results
             * as a List<Movie>. Note the *await* keyword!
             */
            var movies = await _moviesCollection.Find(betterFilter).ToListAsync();
