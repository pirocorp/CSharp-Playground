...
namespace M220N.Repositories
{
    public class MoviesRepository
    {
        ...
        {
            // var filter = $@"{{countries:{{$elemMatch:{{$in:[""{string.Join("\",\"", countries)}""]}}}}}}";

            //var countryFilters = countries.Select(c => Builders<Movie>.Filter.AnyEq(x => x.Countries, c));

            //var filter = Builders<Movie>.Filter.Or(countryFilters);

            var filter = Builders<Movie>.Filter.In(nameof(Movie.Countries).ToLower(), countries);

            var sortByTitleDescending = Builders<Movie>.Sort
                .Descending(m => m.Title);

            var projection = new FindExpressionProjectionDefinition<Movie, MovieByCountryProjection>(
                p => new MovieByCountryProjection
                {
            ...
            int page = 0, params string[] genres)
        {
            ...
            return await _moviesCollection
                .Find(Builders<Movie>.Filter.In(nameof(Movie.Genres).ToLower(), genres))
                .Limit(limit)
                .Skip(page * limit)
                .Sort(sort)
                .ToListAsync(cancellationToken);
