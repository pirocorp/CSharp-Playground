namespace DeepEquality.Models
{
    ...
    public class DeepEqualityResult
    {
        ...
        public DeepEqualityResult(Type expected, Type actual)
        {
            ...
            var initialExpectedName = this.expectedType?.ToFriendlyTypeName();
            var initialActualName = this.actualType?.ToFriendlyTypeName();
        ...
        public override string ToString()
        {
            ...
            var singlePathPart = this.expectedPathParts.Count == 1;
            var pathMessage = string.Empty;

            if (singlePathPart)
            {
                if (this.expectedType != null && this.actualType != null && this.expectedType != this.actualType)
                {
